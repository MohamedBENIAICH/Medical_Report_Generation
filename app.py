...
from database import (
    create_user, 
    verify_user_by_email as verify_user,
    verify_email,
...
GOOGLE_CLIENT_ID = os.getenv("GOOGLE_CLIENT_ID", "1093379467339-kg7o7hbrnhn2rjmpr294k88fo5h7dpmh.apps.googleusercontent.com")
        ...
        with st.form("login_form"):
            ...
            if submit:
                ...
                if user:
                    if not user.get('email_verified', False):
                        st.markdown("""
                            <div class='error-message'>
                                <span class='medical-icon'>⚠️</span> Please verify your email before logging in.
                            </div>
                        """, unsafe_allow_html=True)
                        return
                    st.session_state.authenticated = True
                    ...
                    st.markdown("""
                        <div class='error-message'>
                            <span class='medical-icon'>⚠️</span> Invalid email or password
                        </div>
                    """, unsafe_allow_html=True)
...
def main():
    ...
    verification_token = st.query_params.get("verify", [None])[0]
    reset_token = st.query_params.get("reset", [None])[0]
    ...
    if 'google_token' in st.session_state or st.query_params.get('google_token'):
        st.write("Processing Google Sign-In...")
        token = st.session_state.get('google_token') or st.query_params.get('google_token')[0]
